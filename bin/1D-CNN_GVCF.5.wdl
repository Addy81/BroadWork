...
workflow Cram2FilteredVcf {
    ...
    scatter (calling_interval in SplitIntervals.interval_files) {
        ...
        call GenotypeGVCFs {
            input:
                input_gvcf = RunHC4.raw_vcf,
                input_gvcf_index = RunHC4.raw_vcf_index,
                reference_fasta = reference_fasta,
                reference_dict = reference_dict,
                reference_fasta_index = reference_fasta_index,
                output_prefix = output_prefix,
                ...
                gatk_override = gatk_override,
                preemptible_attempts = preemptible_attempts,
                disk_space_gb = round(ref_size + additional_disk)

        }

        call CNNScoreVariants {
            input:
                input_vcf = GenotypeGVCFs.genotyped_vcf,
                input_vcf_index = GenotypeGVCFs.genotyped_vcf_index,
...
#workflow cnn_variant_common_tasks {}
...
task RunHC4 {
    ...
    output {
        ...
        File raw_vcf = "${output_prefix}_hc4.g.vcf.gz"
        File raw_vcf_index = "${output_prefix}_hc4.g.vcf.gz.tbi"
    }
    runtime {
        docker: "${gatk_docker}"
        memory: machine_mem + " MB"
...
task GenotypeGVCFs {
    ...
    command {
    	set -e export GATK_LOCAL_JAR=${default="/root/gatk.jar" gatk_override}
        gatk --java-options "-Xmx${command_mem}m" GenotypeGVCFs -R ${reference_fasta} -V ${input_gvcf} -O ${output_prefix}_hc4.vcf.gz
    }

    output {
        File genotyped_vcf = "${output_prefix}_hc4.vcf.gz"
        File genotyped_vcf_index = "${output_prefix}_hc4.vcf.gz.tbi"
    }
